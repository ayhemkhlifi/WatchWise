...
function Card(props) {
  const { result, index, type } = props;
  
  // Determine the path based on the type prop
  const detailPath = type === 'tv' ? `/tv/${result.id}` : `/movie/${result.id}`;
    ...
    >
      <Link to={detailPath}>
        <img
          ...
          alt={result.title || result.name} // Use result.name for TV shows
